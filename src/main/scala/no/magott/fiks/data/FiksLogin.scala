...
class FiksLogin {
  ...
  def login(username:String, password:String) = {
    ...
    val response = Jsoup.connect("https://fiks.fotball.no/Fogisdomarklient/Login/Login.aspx")
      ...
      .timeout(10000)
      .execute()
    Console println response.cookies()
    Console println response.statusCode()
  ...
  def availableMatches(loginCookie: (String, String)) = {
    ...
    val matchElements = availableMatchesDoc.select("tbody > tr")
    val matchList = matchElements.listIterator().asScala

    matchList.drop(1).foreach(println)
  }
